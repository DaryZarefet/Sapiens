...
export const PanelOptions = ({ title, buttons }: PanelOptionsProps) => {
  return (
    <div className="animate-in fade-in duration-150 translate-y-2 absolute top-full mt-2 right-0 w-60 bg-surface text-black shadow-lg p-2 z-50 pointer-events-auto border border-gray-300 rounded-lg">
      <div className="text-sm font-semibold px-2 pb-2 bg-surface-2 border-b border-[var(--color-border)]">
        {title}
      </div>
        ...
        {buttons.map((button) =>
          button.onClick ? (
            <button
              ...
              className="flex items-center gap-5 w-full text-left px-2 py-1 hover:bg-gray-100 dark:hover:bg-surface-2 rounded-md transition"
            >
